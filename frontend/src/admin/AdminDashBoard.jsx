...
const AdminDashBoard = () => {
    ...
    datasets: [
      {
        ...
        data: [
          certificates.filter(cert => new Date(cert.endDate).getFullYear() === 2020).length,
          certificates.filter(cert => new Date(cert.endDate).getFullYear() === 2021).length,
          certificates.filter(cert => new Date(cert.endDate).getFullYear() === 2022).length,
          certificates.filter(cert => new Date(cert.endDate).getFullYear() === 2023).length,
          certificates.filter(cert => new Date(cert.endDate).getFullYear() === 2024).length,
        ],
        borderColor: "#4BC0C0",
  ...
  return (
    <div className={`p-6 min-h-screen transition-colors duration-300 ${darkMode ? "dark bg-gray-900" : "bg-gray-50"}`}>
      <div className="flex justify-between items-center mb-8">
        ...
        >
          {darkMode ? <span className="flex items-center"><FiSun size={20} className="mr-2" /> Light Mode</span> : 
                      <span className="flex items-center"><FiMoon size={20} className="mr-2" /> Dark Mode</span>}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          { title: "Total Certificates", value: certificates.length, color: "blue" },
          { title: "Colleges", value: collegeCount, color: "green" },
          { title: "Departments", value: departmentCount, color: "purple" },
          { 
            title: "This Year", 
            value: certificates.filter(cert => new Date(cert.endDate).getFullYear() === new Date().getFullYear()).length,
            color: "orange" 
          }
        ].map((card, index) => (
          <div 
          ...
          >
            <h3 className={`text-gray-500 font-medium ${darkMode ? "text-gray-400" : ""}`}>{card.title}</h3>

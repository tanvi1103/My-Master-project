  ...
  try {
    ...
    if (!admin) {
      return res
        .status(401)
        .json({
          message: `Invalid credentials, Admin not found with ${email} this email`,
        });
    }
...
const logoutAdmin = (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "Logged out successfully", clearLocalStorage: true });
};
...
const uploadFile = async (req, res) => {
  try {
    ...
    if (existingCertificates.length > 0) {
      fs.unlinkSync(req.file.path);
      const existingIDs = existingCertificates.map(
    ...
    try {
      ...
      if (mongoError.code === 11000) {
        ...
        if (mongoError.writeErrors) {
          duplicateIds = mongoError.writeErrors.map(
            (err) => err.err.op.certificateID,
          );
...
const getRegistrarUsers = async (req, res) => {
  try {
    ...
    if (!users || users.length === 0) {
      return res
        .status(404)
        .json({ success: false, error: "No registrars found" });
    }
    ...
    if (req.file) {
      try {
        ...
        const relativePath = path
          .relative(publicDir, req.file.path)
          .replace(/\\/g, "/");
